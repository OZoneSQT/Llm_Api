...
from Training.interface_adapters.controllers.model_training_controller import train
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description='Train a causal language model using the configured datasets.')
    parser.add_argument('--config', type=Path, default=_default_config_path(), help='Path to training configuration JSON.')
    parser.add_argument(
        '--dataset-list',
        type=Path,
...
def main(argv: list[str] | None = None) -> None:
    ...
    print(f'Log file written to {result.log_path}')
