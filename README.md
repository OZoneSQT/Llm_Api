...
- Add Runners for execatution of Media generation, Issue to handle is multiply diffrent pipelines, based on usecase, and handle hardware limitations.
- Add alternativ cache storage location, the current downloads uses +1TB, by not using hardcoded patch, see `Training/domain/tools/paths.py` and `Training/tools/paths.py`.
